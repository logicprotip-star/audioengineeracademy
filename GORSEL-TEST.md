...
| 8 | Geri bildirimde kulak simgesi bazı modlarda yok | ✅ **KAPANDI, G254-G257 (16.08).** G81'de SADECE Frekans Bulma'da vardı (Logic'in isteği, gerekçe: "iki frekans yan yana" — bu gerekçe OLCUM-KULAK-16-08.md'de sorgulandı, Frekans Çakışması Aşama 1'in de aynı "gizli frekans + seçilen frekans" şekline sahip olduğu görüldü). G254-G257'de 7 yere daha eklendi: Kesim Noktası, dB Seviyesi, Pan Konumu, Stereo Genişlik, Boost/Cut katman 2-3, Frekans Çakışması Aşama 1+3 — artık 12 modun 8'inde/parçasında var. Kompresör/Reverb/Saturation&Distortion'da BİLEREK YOK (kendi A/B/C karşılaştırma araçları zaten var, gerek yok), Tonal Denge'de BİLEREK YOK (çok boyutlu, ayrı bir "referans eğri" çözümü gerekir, bkz. madde 12) |
...
| 59 | **Mono dosyada UYARI YOK** — Stereo Genişlik'e mono AIF yüklendi, kilitlenme olmadı, oynattı ama **"bu dosya mono, stereo ölçüm anlamsız" uyarısı çıkmadı.** Kullanıcı mono dosyayla stereo genişlik tahmin etmeye çalışıyor — soru anlamsız, hep aynı cevap. Aynı sorun Araçlar'daki stereo/mid-side ölçümlerinde de olabilir. **Etkilenen yerler:** Stereo Genişlik modu, Pan Konumu modu, Araçlar → stereo ölçümleri | 🔧 düzeltilecek |
...
| 12 | **KARŞILAŞTIRMALI DİNLETME** — yanlış cevapta kullanıcı kendi seçtiğini ve doğruyu arka arkaya dinleyebilsin. `TAM-LISTE.md`'de ileri sürüm fikri olarak kayıtlıydı, madde 8 "12 modun hiçbirinde yok" diyordu — bu iki madde ÇELİŞİYORDU (OLCUM-KULAK-16-08.md'de bulundu). Gerçek: Frekans Bulma'da G81'den beri VARDI, madde 8 bunu atlamıştı. | ✅ **KISMEN KAPANDI, G254-G257 (16.08).** 8 mod/aşamada (Frekans Bulma + Kesim Noktası/dB Seviyesi/Pan Konumu/Stereo Genişlik/Boost-Cut katman 2-3/Frekans Çakışması Aşama 1+3) kulak butonlarıyla ÇÖZÜLDÜ — "senin cevabın" vs "doğru cevap" ayrı ayrı dinlenebiliyor. Kompresör/Reverb/Saturation&Distortion'da ZATEN KENDİ A/B/C karşılaştırma aracı var (seçtiğin harf + doğru harfi art arda çaldırma fikri OLCUM-KULAK-16-08.md'de not edildi, KODLANMADI — 1.1'e bırakılabilir). Tonal Denge AYRI, çok-boyutlu bir çözüm gerektiriyor (tam EQ eğrisi karşılaştırması, Tools'un "referans eğri" özelliğine benzer) — KODLANMADI, 1.1'e bırakılabilir |
